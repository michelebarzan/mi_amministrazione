...
async function popupPermessiUtente(JSONutente)
{
    ...
    var dirty_permessi_pagine_applicazioni=[];
    permessi_pagine.forEach(function (pagina)
    {
        dirty_permessi_pagine_applicazioni.push(pagina.applicazione);
    });
    var permessi_pagine_applicazioni = [];
    $.each(dirty_permessi_pagine_applicazioni, function(i, el){
        if($.inArray(el, permessi_pagine_applicazioni) === -1) permessi_pagine_applicazioni.push(el);
    });
    ...
    permessi_pagine_applicazioni.forEach(function (applicazione)
    {
        ...
        permessi_pagine.forEach(function (pagina)
        {
            if(pagina.applicazione==applicazione)
            {
                var row=document.createElement("div");
                row.setAttribute("class","popup-foto-ordini-row");
